...
def estimate_foot_contacts(
    ...
    # WildRobot joint order:
    # 0: left_hip_pitch, 1: left_hip_roll, 2: left_knee_pitch, 3: left_ankle_pitch
    # 4: waist_yaw
    # 5: right_hip_pitch, 6: right_hip_roll, 7: right_knee_pitch, 8: right_ankle_pitch
    
    left_hip_pitch = dof_pos[:, 0]
    right_hip_pitch = dof_pos[:, 5]
    
    # Simple heuristic: foot in contact when leg is behind (negative hip pitch)
    # and knee is relatively straight (small knee angle)
    left_knee = dof_pos[:, 2]
    right_knee = dof_pos[:, 7]
